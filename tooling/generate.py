#!/usr/bin/env python3
"""Generate data/problems.json and launchpad.html from the question-bank repo.

Source of truth = each topic's LEARNING.md (study order) + LeetCode difficulties.
Run:  QB=/path/to/question-bank python3 tooling/generate.py
"""
import re, os, json, html, sys

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
QB = os.environ.get("QB", "/tmp/question-bank")
DIFF_JSON = os.environ.get("DIFF_JSON", "/tmp/leet_diff.json")
EXT_ID = "prateek.dsa-bank"     # must match tooling/dsa-bank/package.json
...
.bar{{display:flex;gap:10px;flex-wrap:wrap;padding:14px 24px;background:#DCEBF7;position:sticky;top:0;z-index:5;border-bottom:1px solid var(--line)}}
...
.stat{{font-size:13px;color:#0B3D5C;font-weight:600;align-self:center}}
...
.desc{{color:var(--mut);font-size:12px;max-width:340px}}
...
<p>Click <b>Open</b> to scaffold &amp; open the problem in VS Code · {must} must-do · open the
<code>DSA_QUESTIONS</code> repo in VS Code first, with the <b>dsa-bank</b> extension installed.</p></header>
<div class="bar">
  ...
  <select id="pr" onchange="flt()"><option value="">All</option>
    <option>Must-Do</option><option>Optional</option></select>
  <span class="stat" id="cnt"></span>
</div>
<table id="t"><thead><tr>
<th>#</th><th>Stage</th><th>Topic</th><th>Pattern</th><th>Problem</th>
<th>Difficulty</th><th>Priority</th><th>Open</th><th>Links</th><th>Notes</th>
    ...
    for r in rows:
        ...
        blob = " ".join(filter(None, [
            r["title"].lower(), r["topic"].lower(), r["pattern"].lower(), r["difficulty"].lower(),
            r["priority"].lower(), r["stageName"].lower()]))
        parts.append(f"""<tr data-s="{html.escape(r['stageName'])}" data-d="{r['difficulty']}" data-p="{r['priority']}" data-b="{html.escape(blob)}" style="--t:{STAGE_TINT[r['stage']]}">
<td>{r['order']}</td>
...
<td><a class="btn open" href="{html.escape(vscode_uri(r))}">Open ▸</a></td>
<td>{lc} {card}</td>
...
function flt(){
 ...
 const s=document.getElementById('st').value, d=document.getElementById('df').value, p=document.getElementById('pr').value;
 let n=0; document.querySelectorAll('#t tbody tr').forEach(tr=>{
   const ok=(!q||tr.dataset.b.includes(q))&&(!s||tr.dataset.s===s)&&(!d||tr.dataset.d===d)&&(!p||tr.dataset.p===p);
...
if __name__ == "__main__":
    ...
    json.dump(rows, open(os.path.join(ROOT, "data", "problems.json"), "w"), indent=2)
    open(os.path.join(ROOT, "launchpad.html"), "w").write(build_html(rows))
    est = sum(1 for r in rows if r["difficultyEst"])
    print(f"wrote data/problems.json + launchpad.html  ({len(rows)} problems, "
          f"{len(rows)-est} real difficulty, {est} estimated)")
